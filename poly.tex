...
Unlike the case of complex multiplication, left $\O$-ideal classes do
not form a group. %
Nevertheless, thanks to Eichler's mass formula, we obtain the exact
size of the left ideal class set and thus of the supersingular isogeny
class. %
...
\end{proposition}
\begin{proof}
  If $p = 2$, then $E_0$ is the curve $y^2 = x^3 + x$ with
  $j$-invariant 1728, $(1 + i + j + ij)/2$ is one of its automorphisms of order
  ...
  $j$-invariant $8000$. %
  This curve has a single cyclic group of order $2$ over $ℚ$, the
  kernel of an endomorphism of degree $2$, take its reduction for
  $i$. %
...
%     edge[loop left,in=135,out=215,looseness=8] node[left]{\(ω\)} (E0);
    \draw[-latex] (E0.south) arc (330:30:.6);
    \draw (E0) +(-1.4,0) node {\(ω\)} ;
  \end{tikzpicture}
\end{equation*}
