...
\begin{proposition}[V\'elu's formulas]
  ...
  The separable isogeny $ϕ:E\to E'$ of kernel $G$ can be written as
  \begin{equation*}
    ϕ(P) = \left(
...
\begin{theorem}[Graphs of horizontal isogenies are expanders]
  ...
  whose endomorphism ring is isomorphic to $\O$, and whose edges are
...
\begin{definition}[Schreier graph]
  ...
  \begin{align*}
    ...
    (g,x) &\mapsto g·x
  \end{align*}
  such that $g·x=x$ if and only if $g=1$, and $g·(h·x)=(gh)·x$, for
  all $g,h∈G$ and $x∈X$. %
  Let $S⊂G$ be a \emph{symetric} subset, i.e. one not containing $1$
  ...
  if and only if $s·x=x'$ for some $s∈S$.
\end{definition}
...
acts naturally on $G$ by the law $a·g=g^a$ for any $g∈G$ and
$a∈(ℤ/nℤ)^{×}$. %
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[xshift=4cm,yshift=1cm]
      \draw[blue] (0,0) -- (0.5,0) (0.5,0) node[anchor=west] {$x \mapsto x^{2}$};
      \draw[red] (0,-1) -- (0.5,-1) (0.5,-1) node[anchor=west] {$x \mapsto x^{3}$};
      \draw[green] (0,-2) -- (0.5,-2) (0.5,-2) node[anchor=west] {$x \mapsto x^{5}$};
    \end{scope}
  \end{tikzpicture}
...
By slightly generalizing Couveignes' definition~\cite{Couv}, we will
now show how to construct a key exchange protocol based on this
Schreier graph. %
...
\begin{figure}
  \begin{subfigure}{0.45\textwidth}
    ...
    \end{tikzpicture}
    \caption{Diffie-Hellman based on group action: $g,h$ are random
      elements of an abelian group $G$, acting on a set $X\ni x_0$.}
    \label{fig:dh-action}
  \end{subfigure}
  %%
  \hfill
  ...
  \begin{subfigure}{0.45\textwidth}
    ...
    \begin{tikzpicture}
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{Two isogeny cycles superimposed}
    \label{fig:rs}
  \end{subfigure}
  \caption{Key exchange based on ordinary isogeny graphs.}
\end{figure}
...
\begin{figure}
  ...
  \begin{tikzpicture}
    \begin{scope}
      \draw (0,1.2) node[anchor=east,blue] {$\ker ϕ=〈P〉⊂ E[\ell_A^{e_A}]$};
      \draw (0,0.4) node[anchor=east,red] {$\ker ψ=〈Q〉⊂ E[\ell_B^{e_B}]$};
      \draw (0,-0.4) node[anchor=east,blue] {$\ker ϕ' = 〈\rd{ψ}(P)〉$};
      \draw (0,-1.2) node[anchor=east,red] {$\ker ψ' = 〈\bl{ϕ}(Q)〉$};
    \end{scope}
    \begin{scope}[xshift=4.5cm]
      ...
      \path[->,blue] (E) edge node[auto] {$ϕ$} (Es);
      \path[->,blue] (Ep) edge node[auto,swap] {$ϕ'$} (Eps);
      \path[->,red] (E) edge node[auto,swap] {$ψ$} (Ep);
      \path[->,red] (Es) edge node[auto] {$ψ'$} (Eps);
    \end{scope}
  \end{tikzpicture}
  
  \caption{Commutative diagram of isogenies of coprime degree}
...
\begin{figure}
  ...
  \begin{tikzpicture}
    \node[matrix of nodes, ampersand replacement=\&, column sep=4mm, row sep=2cm] (diagram) {
      \& |(1)| $E$ \\
      |(a)| \parbox{1.5cm}{$E/〈\bl{R_A}〉$\\{\footnotesize $\bl{ϕ(}\rd{P_B}\bl{)}\\\bl{ϕ(}\rd{Q_B}\bl{)}$}} \& \&
      |(b)| \parbox{1.5cm}{$E/〈\rd{R_B}〉$\\{\footnotesize $\rd{ψ(}\bl{P_A}\rd{)}\\\rd{ψ(}\bl{Q_A}\rd{)}$}}\\
      \normalsize $\frac{E/〈\bl{R_A}〉}{\bl{ϕ(}\rd{R_B}\bl{)}} \simeq$ \&
      |(ab)|  $E/〈\bl{R_A},\rd{R_B}〉$ \&
      \normalsize $\simeq \frac{E/〈\rd{R_B}〉}{\rd{ψ(}\bl{R_A}\rd{)}}$\\
    };
    \small
    \path[blue] (1) edge node[auto,swap](phia) {$ϕ$} (a);
    \path[red] (1) edge node[auto](phib) {$ψ$} (b);
    \path[red] (a) edge node[auto,swap](psia){$ψ'$} (ab);
    \path[blue] (b) edge node[auto](psib){$ϕ'$} (ab);
    \path[dashed,->] (phia) edge node[auto]{\footnotesize $\bl{ϕ(}\rd{R_B}\bl{)}$} (psia);
    \path[dashed,->] (phib) edge node[auto,swap]{\footnotesize $\rd{ψ(}\bl{R_A}\rd{)}$} (psib);
  \end{tikzpicture}

  \caption{Schematics of SIDH key exchange}
  \label{fig:sidh}
\end{figure}
...
%  LocalWords:  supersingular irreducibility cryptographically
%  LocalWords:  expander expanders Schreier
