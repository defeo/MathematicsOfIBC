...
\DeclareMathOperator{\Hom}{Hom} % endomorphism ring
...
project. %
Valerio Talamanca, Marusia Rebolledo, Carlos Alberto Trujillo Solarte
and the Universidad del Cauca. %
Damien Stehlé, El Mamoun Souidi and the University of Rabbat.
...
Any element of a quadratic number field must satisfy a quadratic
equation. %
Those that satisfy an equation \emph{with integer coefficients} can be
seen as a generalization of the ring $ℤ$ of integers inside $ℚ$.
...
TODO: ramification, division algebra...

\paragraph{Lattices and ideals} TODO: index

\paragraph{Orders} TODO
...
A classical example of order is the ring of integers $\O_K$ of a
number field $K$. %
...
Let $K$ be a quadratic imaginary field. %
...
Determining the exact order isomorphic to $\End(E)$ is much more
complicated and we shall come back to it in
Sections~\ref{sec:ell-isogeny-graphs} and~\ref{}.
...
We recall some basic concepts of graph theory. %
A directed multi-graph $G$ is a pair $(V,E)$ where $V$ is a finite set
of \emph{vertices} and $E∈ℕ^{V×V}$ is a multi-set of ordered pairs
called \emph{edges}. %
When $E$ is a simple set, i.e.\ $E∈\{0,1\}^{V×V}$, we recover the
usual definition of a graph. %
Given any edge $(v,v')∈E$, we say that $v'$ is \emph{connected} to
$v$. %
...
$v\to v_1\to\cdots\to v'$ such that any two consecutive vertices are
neighbors. %
The \emph{degree} of a vertex is the number of edges departing from
it; a graph where every edge has degree $k$ is called
\emph{$k$-regular}. %
The \emph{adjacency matrix} of a graph $G=(V,E)$ is the $|V|×|V|$
matrix with columns and rows indexed by the vertices,  where the
$(i,j)$-th entry is the multiplicity of the edge $(v,v')$.

We will call a multi-graph \emph{undirected} if whenever $(v,v')∈E$,
then $(v',v)∈E$ too; note that we do not ask the multiplicities of
$(v,v')$ and $(v',v)$ to be the same. %
The \emph{distance} between two vertices in an undirected multi-graph
is the length of the shortest path between them; if there is no such
path, the vertices are said to be at infinite distance. %
An undirected multi-graph is called \emph{connected} if any two
vertices have a path connecting them; it is called \emph{disconnected}
otherwise. %
The \emph{diameter} of a connected multi-graph is the largest of all
distances between its vertices. %

\begin{definition}[Isogeny graph]
  An \emph{isogeny graph} is a multi-graph whose vertices are
...
For this reason, isogeny graphs are usually undirected. %
...
$π(\ker ϕ)=\ker ϕ$, which suggests looking at the restriction of $π$ to
$E[ℓ]$. %
...
To address this question, we shall introduce the theory of complex
multiplication.

\section{Complex multiplication}
\label{sec:compl-mult}

We now introduce one of the most powerful tools for the study of
isogeny graphs. %
...
TODO: Marc, reorganize/rewrite/expand
