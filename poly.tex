...
graphs. %
Graphs of supersingular isogenies have two main features for
constructing key exchange protocols, compared to ordinary graphs. %
First, one isogeny degree is sufficient to obtain an expander graph;
this has the potential to produce more efficient protocols. %
Second, there is no action of an Abelian group, such as $\Cl(\O)$, on
it; it seems thus harder to use quantum computers to solve the isogeny
path problem in them.
...
quaternion algebra and their maximal orders. %
We address the interested reader to~\cite{belding08-thesis}. %
Instead, we describe here a key exchange protocol, only using the
language of isogenies.
