...
\begin{exercise}[Pohlig-Hellman algorithm]
...
\begin{theorem}
  ...
  and the group of maps $ϕ_α$ for all $α$ such that $αΛ⊂Λ'$.
\end{theorem}
\begin{proof}
...
\begin{proposition}
  ...
  $E$. %
  There are a unique elliptic curve $E'$, up to composition with an
  isomorphism $E'≃E''$, and a unique separable isogeny $ϕ$, such that
  $\ker ϕ=G$ and $ϕ:E\to E'$. %
\end{proposition}
\begin{proof}
...
The proposition justifies introducing the notation $E/G$ for the
...
\begin{definition}[Order]
  ...
  An \emph{order} $\O⊂K$ is a subring of $K$ that is a $ℤ$-lattice of
  maximal dimension.
\end{definition}
...
two curves are $ℓ$-isogenous; by the dual isogeny theorem, this is
...
\begin{proposition}
  Let $E$ be an ordinary elliptic curve over a finite field $k$. %
  Let $\O$ be its endomorphism ring, $f$ its conductor, $D$ its
  discriminant, $π$ the Frobenius endomorphism, $f_π$ the conductor of
  $ℤ[π]$. %
...
\begin{theorem}[Graphs of horizontal isogenies are expanders]
  ...
  that $G$ is a two-sided $ε$-expander.
\end{theorem}
\begin{proof}
  See~\cite{JMV}.
\end{proof}
