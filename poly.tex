...
her choosing a secret cyclic subgroup $〈P〉⊂E[ℓ_A^{e_A}]$. %
If we let Alice choose one such subgroup, and Bob choose similarly a
secret $〈Q〉⊂E[ℓ_B^{e_B}]$, then there is a well defined subgroup
$〈P〉+〈Q〉=〈P,Q〉$, defining an isogeny to $E/〈P,Q〉$. %
Since we have taken care to choose $ℓ_A≠ℓ_B$, the group $〈P,Q〉$ is
...
\begin{figure}
  ...
  \begin{tikzpicture}
    \begin{scope}
      \draw (0,1.2) node[anchor=east] {$\bl{\ker ϕ=〈P〉}⊂ E[\ell_A^{e_A}]$};
      \draw (0,0.4) node[anchor=east] {$\rd{\ker ψ=〈Q〉}⊂ E[\ell_B^{e_B}]$};
      \draw (0,-0.4) node[anchor=east] {$\ker ϕ' = 〈\rd{ψ}\bl{(P)}〉$};
      \draw (0,-1.2) node[anchor=east] {$\ker ψ' = 〈\bl{ϕ}\rd{(Q)}〉$};
    \end{scope}
    \begin{scope}[xshift=4.5cm]
      ...
      \path[->] (Ep) edge node[auto,swap] {$ϕ'$} (Eps);
      \path[->,red] (E) edge node[auto,swap] {$ψ$} (Ep);
      \path[->] (Es) edge node[auto] {$ψ'$} (Eps);
    \end{scope}
  \end{tikzpicture}
...
choose random cyclic subgroups $〈P〉$ and $〈Q〉$ in some \emph{large
  enough} torsion groups, and exchange enough information to both
compute $E/〈P,Q〉$, without revealing their respective secrets. %
We are faced with two difficulties, though:
\begin{enumerate}
\item The points of $〈P〉$ (or $〈Q〉$) may not be rational. %
...
\item The diagram in Figure~\ref{fig:sidh-diag} shows no way by which
  Alice and Bob could compute $E/〈P,Q〉$ without revealing their
