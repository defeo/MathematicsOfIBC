...
\begin{figure}
  ...
  \begin{tabular}{l *{2}{p{32ex}<{\centering}}}
    ...
    Parameters & \multicolumn{2}{l}{Primes $ℓ_A,ℓ_B$, and a prime $p=ℓ_A^{e_A}ℓ_B^{e_B}f∓1$,}\\
               ...
               & \multicolumn{2}{l}{A basis $〈P_B,Q_B〉$ of $E[ℓ_B^{e_B}]$.}\\
    \hline
    Secret key & \multicolumn{2}{l}{An isogney $α:E\to E/〈A〉$ of degree $ℓ_A^{e_A}$.}\\
    \hline
...
\end{itemize}
When he receives Alice answer, he accepts only if the points do define
isogenies between the curves $E,E/〈A〉,E/〈B〉,E/〈A,B〉$.  The
protocol is summarized in Figure~\ref{fig:sidh-zk}.
...
cheater's chance of success decreases exponentially. %
...
assumption, named DSSP, to prove zero knowledge. %
...
and zero-knowledge under the SSDDH\footnote{Actually, a weaker
  assumption named CSSI.} and DDSP assumptions. %
In the same paper, a variant of the protocol where Bob challenges
Alice to reveal one out of three informations, namely one of
$Β,α(B),β(A)$ is also hinted at. %
This variant is less efficient, since a cheater has $2/3$ chances of
success, however its security relies on the isogeny walk problem,
rather than SSDDH.

From the protocol, we can
