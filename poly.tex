...
When the kernel of a separable isogeny is cyclic we call it a
\emph{cyclic} isogeny, and when then kernel is isomorphic to $ℤ/ℓℤ$ we
call it an $\ell$-isogeny.
...
\begin{example}
  ...
  \begin{equation}
    ...
    \end{aligned}
  \end{equation}
  is a separable $2$-isogeny between curves defined over $ℚ$. %
  ...
  example, is a $2$-to-$1$ (algebraic) group morphism of
...
\paragraph{Representing and computing isogenies.}
...
can represent isogenies and evaluate them at arbitrary points using an
amount of field operations polynomial in their degree. %
...
degree $N$ can thus be stored and evaluated in time polynomial in
$\log(N)$ and the largest prime factor of $N$.

We will later discover more efficient ways to store and evaluate
isogenies, which motivates abstracting away the details of how an
isogeny is actually represented into a definition that focuses on what
one can actually do with the representation.

\begin{definition}[Isogeny representation]
  \label{def:isog-repr}
  Let $\F_q$ be a finite field. %
  An \emph{isogeny evaluator} \IR{} is a pair of polynomial-time
  algorithms:
  \begin{itemize}
  \item $\IR[.valid](\phi)$ taking as input a string
    $\phi\in\{0,1\}^*$ and outputting either a symbol $\bot$ or a
    triple $(E, E', d)$; in the latter case, $E$ and $E'$ are elliptic
    curves defined over $\F_q$ and there exists an isogeny
    $\phi_{\IR}: E\to E'$ of degree $d$.
  \item $\IR[.eval](\phi, P)$ taking as input a string
    $\phi\in\{0,1\}^*$ and a point $P\in E(\F_{q^k})$; if
    $\IR[.valid](\phi) = (E, E', d)$ it outputs the image point
    $\phi_{\IR}(P)\in E'(\F_{q^k})$, otherwise the output is
    undefined.
  \end{itemize}

  In the case that \IR[.valid] does not output $\bot$ and that $\phi$
  is of size polynomial in $\log(q)$ and $d$, the string $\phi$ is
  called an \emph{efficient representation} of $\phi_{\IR}$ (for the
  evaluator \IR).  When the evaluator is clear from context, we will
  omit the subscript in $\phi_{\IR}$ and write $\phi$ for both the
  representation and the isogeny it represents.
\end{definition}
...
When we restrict to isogenies of a prescribed prime degree $ℓ$, we say
that two curves are $ℓ$-isogenous; by the dual isogeny theorem, this
is a symmetric relation. %
Remark that being $ℓ$-isogenous is also well defined up to
isomorphism.

Let us start with the local structure of the graph: given an elliptic
curve $E$ and a prime $ℓ$, how many isogenies of degree $ℓ$ have $E$
as domain? %
...
If $n$ is the smallest exponent such that $π^n$ acts like a scalar
multiplication, i.e.\ if $n$ is the order of $π$ in $\PGL_2(\F_ℓ)$,
then $\F_{q^n}$ is the smallest field where all $ℓ$-isogenies of $E$
are defined. %
...
But what about the global structure of the graph? %
...
\begin{figure}
  ...
  \begin{algorithmic}[1]
    ...
    \STATE Set $K = [N/ℓ]P$;
    \UNTIL{$K$ is not the point at infinity;}
