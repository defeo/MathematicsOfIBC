...
\author{Luca De Feo\\[1em]
  \textcolor{gray}{Université de Versailles, Versailles, France}\\
...
\date{
  \begin{tabular}{r @{~--~} l}
    ...
    v3 & September 2022, Bonn, Germany
  \end{tabular}
}
...
This third revision was written in the summer of 2022 for the school
\href{https://cryptabit.bit.uni-bonn.de/2022}{crypt@b-it} in Bonn,
Germany. %
It fixes several mistakes, adds extra material, and, obviously,
moves on from SIDH.

Over the course of these 5 years, isogeny based cryptography has
...
doubt~\cite{EPRINT:CasDec22,EPRINT:MaiMar22,EPRINT:Robert22}. %
Today isogeny based cryptography is too vast to be taught in a single
week of lectures, hence these notes do not attempt at drawing a
complete picture. %
...
mentioned above. %
For having organized such wonderful events and supported my lectures,
I would like to thank CIMPA, the Écoles Mathématiques Africaines, the
...
of Würzburg. %
Michael Nüsken, the b-it (Bonn-Aachen International Center for
Information Technology), CASA (Cyber Security in the Age of
Large-Scale Adversaries) and the smashHit project.

Many students have contributed by pointing out mistakes and helping
fixing them. %
I would like to thank, in particular, Simon Masson, Marcel Müller,
Martin Strand and Vadym Fedyukovych, and apologize to all those
students whose name I've forgot.
...
When $k$ is not algebraically closed, it is customary to write $E[m]$
for the $m$-torsion subgroup of $E(\bar{k})$, i.e.\ the torsion points
in the algebraic closure. %
$E[m]$ may or may not be fully contained in $E(k)$, it is easy to see,
however, that it will always be contained in an extension of $k$ of
finite degree.
...
If you feel uncomfortable with rings and modules, just think of vector
spaces over a field.
...
We know that (in a sufficiently large finite extension of $k$) the
torsion subgroup $E[m]$ is isomorphic to $(ℤ/mℤ)^2$, and is thus a
free $ℤ/mℤ$-module of rank two. %
$E[m]$ comes equipped with a non-degenerate alternating pairing
$e_m:E[m]\times E[m]\to \mu_m$, where $\mu_m\subset\bar{k}$ is the
subgroup of $m$-th roots of unity of (the algebraic closure of) $k$,
called the \emph{Weil pairing}.

But since $\mu_m$ is isomorphic to $ℤ/mℤ$ as $ℤ/mℤ$-module, the Weil
pairing is just a bilinear form in disguise. %
And not just any bilinear form! %
Indeed, under the constraint of being non-degenerate and alternating,
the Weil pairing is, essentially, the $2×2$ determinant form.
...
We skip here details on how the Weil pairing can be computed (see,
e.g.,~\cite{galbraith2012mathematics}).  %
Suffice to say that there is an efficient algorithm (with running time
polynomial in $\log(m)$). %
Other pairings are also defined for elliptic curves, however they are
all related to the Weil pairing, and they will not make a difference
for our purposes, thus we will ignore them. %
...
We conclude with one of the most powerful tools for the study of isogeny
...
discriminant $Δ_K$ of $K$ is a square modulo $p$. %
...
quantum computers. %
In the next sections we shall present two key exchange protocols based
on pseudo-random walks in certain isogeny graphs, which are
conjectured to be resistant to attacks by quantum computers.


\section{Isogeny graphs and discrete logarithms}
\label{sec:isog-graphs-crypt}

Before moving to post-quantum cryptography, we quickly review one of
the first applications of isogeny graphs in cryptography: hardness of
discrete logarithms in families of elliptic curves. %
...
\begin{problem}
  ...
  known degree $ℓ$, find an isogeny $ϕ:E\to E'$ of degree $ℓ$.
\end{problem}

Remark that, up to automorphisms, the isogeny $ϕ$ in the latter
problem is typically unique. %
...
The difficulty of this problem depends on the set which $E$ and $E'$
are drown from, but it is in general a very difficult one, for which
only algorithms exponential in $\log(\#E)$ are known. %
Remark that, in general, $\degϕ$ will be exponential in $\log(\#E)$,
thus the "smooth degree" constraint, which allows the output to be
represented using polynomial space as a composition of small degree
isogenies.
...
discrete logarithms inside some isogeny classes~\cite{jao+miller+venkatesan09}.
...
Stolbunov~\cite{rostovtsev+stolbunov06}, we may define a variant of
Couveignes' HHS key exchange based on walks in a Cayley graph for
$G$. %
As an example, let $G=〈g〉$ be a cyclic group of order $p$, let
...
Then, $G$ minus the identity is a PHS for $(ℤ/pℤ)^{×}$ under the
action
\[e·g_0 = g_0^e\qquad\text{for $e∈(ℤ/pℤ)^×$ and
    $g_0∈G\setminus\{1\}$}.\] %
We may thus define the Schreier graph of
$((ℤ/pℤ)^{×}, S, G\setminus\{1\})$, which is isomorphic to the Cayley
...
difference being that the secret exponents $a,b$ are drawn from a not
necessarily uniform distribution. %
While this example instance is of no practical interest, its
instantiation using a Schreier graph of the HHS $\Ell_q(\O)$ yields a
usable variant of Couveignes' key exchange. %
...
A first is to use Vélu's formulas instead of Elikes' or Couveignes'
algorithm, as first proposed in~\cite{10.1007/978-3-030-03332-3_14}. %
...
This was the main insight that recently lead to the key exchange
protocol CSIDH~\cite{10.1007/978-3-030-03332-3_15}, which is an
efficient variant of the Couveignes--Rostovtsev--Stolbunov system in a
...
In Section~\ref{sec:sidh} we are going to present a different key
exchange protocol, named SIDH, based on supersingular isogeny
graphs. %
...
We end this part with a quick review of the security of the key
exchange protocols presented so far. %
The problem that is often cited as the cornerstone of isogeny based
cryptography is the isogeny path problem: given isogenous curves $E$,
$E'$, find an isogeny of smooth degree between them; however, each
cryptosystem depends on a specific variant of the problem.
...
In the CM case, using the theory of isogeny volcanoes, we can easily
find ascending paths from $E$ and $E'$ to two curves
...
This would be enough information to recover the secret isogeny by
interpolation techniques, however we do not know how to exploit the
...
At any rate, the security of SIDH cannot be founded on the isogeny
path problem. %
Instead, it is necessary to make \emph{ad hoc} assumptions taking into
account the information communicated by the protocol, that go under
the names of CSSI, SSCDH, SSDDH
(see~\cite{jao+defeo2011,defeo+jao+plut12} or SIDH~\cite{SIKE}).
...
% LocalWords:  discriminants homothetic Couveignes
